...
def psm_intro_copy():
    return [
        ...
        html.P([
            ...
            "flags. ",
            html.Strong("Matching is with replacement"),
            "—the same control customer can be the nearest neighbour for more than one treated ",
            "recipient, so the matched-control pool is smaller than the matched-treated pool and ",
            "some control rows are weighted more heavily than others. If even the nearest control ",
            "still looks too different (logit-propensity gap beyond the caliper), we omit that ",
            "shopper from this sensitivity view instead of stretching the comparison. ",
        ...
        html.P([
            ...
            "it doesn't replace the Overview or Tab 3 (OLS) estimates that lean on the ", html.Strong("randomised"),
